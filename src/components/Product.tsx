...
const Product = ({ product, dispatch, REDUCER_ACTIONS, inCart }: PropsType): ReactElement => {
    ...
    const content =
        <article className="card card-compact w-96 bg-base-100 shadow-xl card-actions ">
            <h3 className="text-xl ">{product.name}</h3>
            <img src={img} alt={product.name} className="product__img" />
            <p className="text-xl">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(product.price)}{itemInCart}</p>
